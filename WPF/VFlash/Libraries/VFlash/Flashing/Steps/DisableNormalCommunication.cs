...
namespace VFlash.Flashing {
    public class DisableNormalCommunication : FlashAction {
        public DisableNormalCommunication() {
            IsFunctional = true;
            SuppressBit = false;
        }
        ...
        public override void Execute(FlashActionArgs actionArgs) {
            StartNewAction("Disable Normal Communication", () => {
                byte[] request = (byte[])UDSMessage.DisableNormalCommunication.Clone();
                ...
                if(response != UDSResponseType.SendRequestSuccessful)
                    throw new FlashException("Disable Normal Communication Failed");
                if(!SuppressBit) {
