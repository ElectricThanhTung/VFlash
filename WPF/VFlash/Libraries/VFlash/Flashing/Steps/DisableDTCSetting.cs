...
namespace VFlash.Flashing {
    public class DisableDTCSetting : FlashAction {
        public DisableDTCSetting() {
            IsFunctional = true;
            SuppressBit = false;
        }
        ...
        public override void Execute(FlashActionArgs actionArgs) {
            StartNewAction("Disable DTC Settings", () => {
                byte[] request = (byte[])UDSMessage.DisableDTCSetting.Clone();
                ...
                if(response != UDSResponseType.SendRequestSuccessful)
                    throw new FlashException("Disable DTC Settings Failed");
                if(!SuppressBit) {
