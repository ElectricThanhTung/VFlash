...
namespace VFlash.Flashing {
    public class StartDefaultSession : FlashAction {
        public StartDefaultSession() {
            IsFunctional = true;
            SuppressBit = false;
        }
        ...
        public override void Execute(FlashActionArgs actionArgs) {
            StartNewAction("Start Default Session", () => {
                byte[] request = (byte[])UDSMessage.DefaultSession.Clone();
                ...
                if(response != UDSResponseType.SendRequestSuccessful)
                    throw new FlashException("Start Default Session Failed");
                if(!SuppressBit) {
